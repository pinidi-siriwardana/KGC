...
import { UserPlus, Edit3, Search, Phone, Mail } from 'lucide-react';
...
const fetchGuests = async (searchQuery = "") => {
    ...
    const filteredGuests = statusFilter ? guests.filter((g) => g.status === statusFilter) : guests;

    const handleOpenModal = (guest = null) => {
        if (guest) {
            setEditingGuest(guest);
            setFormData({ full_name: guest.full_name, phone: guest.phone, email: guest.email || '', status: guest.status || 'active' });
        } else {
        ...
        <div className="p-8 bg-slate-50 min-h-screen">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredGuests.map((guest) => (
                            <tr key={guest.guest_id} className="group hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="p-6">
                                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full border ${guest.status === 'active' ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-amber-600 bg-amber-50 border-amber-100'}`}>
                                        {guest.status || 'active'}
                                    </span>
                                </td>
                ...
                {filteredGuests.length === 0 && (
                    <div className="p-20 text-center">
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">
                            {guests.length === 0 ? 'No guests found in the system' : 'No guests match this filter'}
                        </p>
                    </div>
                )}
            </div>
            ...
            >
                <div className="space-y-5">
                    ...
                    {editingGuest && (
                        <div>
                            <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest block mb-2 px-1">Status</label>
                            <select
                                className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:ring-4 focus:ring-slate-900/5 focus:border-slate-200 outline-none transition-all"
