...
const MemberPayments = () => {
    ...
    const handleModalSubmit = async (e) => {
        ...
        if (payingFee) {
            ...
            res = await apiFetch(`/api/member/payments/${payingFee.payment_id}/pay`, { method: 'POST', body });
        } else {
            const body = new FormData();
            body.append('payment_type', formData.payment_type);
            if (formData.payment_type === 'membership_renewal') {
                body.append('membership_type_id', formData.membership_type_id);
            } else {
                body.append('amount_declared', formData.amount_declared);
            }
            if (formData.note) body.append('note', formData.note);
            body.append('receipt', receiptFile);
            res = await apiFetch('/api/member/payments', { method: 'POST', body });
        }
                            ...
                            <div key={r.verification_id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                <div>
                                    <p className="text-slate-900 text-sm font-bold capitalize">
                                        {r.payment_type.replace(/_/g, ' ')}{r.requested_plan_name ? ` — ${r.requested_plan_name}` : ''}
                                    </p>
                    ...
                    <div className="space-y-4">
                        {payingFee ? (
                            ...
                            <>
                                ...
                                {formData.payment_type === 'membership_renewal' ? (
                                    <div className="space-y-1">
