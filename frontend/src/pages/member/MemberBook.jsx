...
const MemberBook = () => {
    ...
    const handleConfirm = async (e) => {
        ...
        if (res.ok) {
            setPending(null);
            setSuccess('Court booked and confirmed.');
            refreshAvailability();
            setTimeout(() => setSuccess(''), 4000);
        } else {
            const err = await res.json();
            if (res.status === 409) refreshAvailability();
            setError(err.message || 'Failed to book this slot.');
        }
    };
    ...
    return (
        <div className="relative space-y-8 animate-in fade-in duration-700">
            <header className="relative z-10">
                <h1 className="text-slate-900 text-4xl font-serif italic">Book a Court.</h1>
                <p className="text-emerald-600 text-[10px] font-black uppercase tracking-[0.3em] mt-2">
                    Free for members — pick an open slot
                </p>
            </header>
                ...
                {pending && (
                    <div className="space-y-3">
                        <p className="text-slate-600 text-sm">
                            Book <span className="font-bold text-slate-900">{pending.court.court_name}</span> on{' '}
