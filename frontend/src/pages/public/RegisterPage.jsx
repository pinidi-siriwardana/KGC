...
const RegisterPage = () => {
  const [formData, setFormData] = useState({
    full_name: '', email: '', phone: '', username: '', password: '', membership_type_id: ''
  });
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/membership-types`)
      .then((res) => res.json())
  ...
  const handleSubmit = async (e) => {
    ...
    if (!receiptFile) {
      setError('Please attach your membership payment slip.');
      return;
    }
        ...
        <div className="flex-1 bg-[#080a0d] p-10 md:p-14 relative">
          ...
          {submitted ? (
            <div className="space-y-4 py-10 text-center">
              <CheckCircle2 className="text-emerald mx-auto" size={40} />
              <p className="text-white text-sm font-bold uppercase tracking-widest">Application Submitted</p>
              <p className="text-white/50 text-xs leading-relaxed max-w-sm mx-auto">
                Your registration and payment slip are pending admin review. You'll be able to sign in once your account is approved.
              </p>
              <Link to="/login" className="inline-block text-[9px] font-black uppercase tracking-widest text-amber hover:text-white transition-colors">
                Return to Sign In
              </Link>
            </div>
          ) : (
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2 group">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" name="full_name" placeholder="Member Name" required
              ...
              <div className="space-y-2 group">
                <label className="text-[9px] font-black uppercase tracking-registry text-emerald/80 ml-1 group-focus-within:text-amber transition-colors">Email</label>
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-amber transition-colors" size={16} />
              ...
              <div className="pt-4 border-t border-white/5 space-y-2 group">
                <label className="text-[9px] font-black uppercase tracking-registry text-emerald/80 ml-1 group-focus-within:text-amber transition-colors">Membership Plan</label>
                <select name="membership_type_id" required value={formData.membership_type_id} onChange={handleChange}
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl py-4 px-6 text-white text-sm outline-none focus:border-amber/50 focus:ring-4 focus:ring-amber/10 transition-all appearance-none cursor-pointer">
                  <option value="" disabled className="text-muted">Select a plan</option>
                  {membershipTypes.map((type) => (
                    <option key={type.membership_type_id} value={type.membership_type_id} className="text-obsidian">
                      {type.name} — LKR {type.price} / {type.duration_months}mo
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-4 px-6">
                      ...
                      <div className="text-left">
                        <p className={`text-[10px] font-black uppercase tracking-widest ${receiptFile ? "text-emerald" : "text-white"}`}>
                          {receiptFile ? "Slip Attached" : "Upload Slip"}
                        </p>
                        <p className="text-[8px] text-white/30 uppercase font-bold tracking-tighter">
                          {receiptFile ? receiptFile.name : "JPG, PNG or PDF (Max 5MB)"}
                        </p>
                      </div>
                    </div>
                  </label>
              ...
              <div className="pt-6 space-y-4">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-emerald hover:bg-emerald/80 text-white font-black uppercase tracking-registry text-[10px] py-5 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl shadow-emerald/20 group disabled:opacity-50 disabled:pointer-events-none"
                >
